...
public class LogParser {
	...
	private static void reportLoggedIn(List<Request> requests) {
		...
		List<Session> sessionList = sessions.entrySet().stream()
				.map(entry -> new Session(entry.getKey(), entry.getValue())).collect(Collectors.toList());
	...
	private static void reportWebsiteRequests(List<Request> requests) {
		...
		Map<String, List<Request>> sessions = requests.stream().filter(request -> !request.isTracking())
				.collect(Collectors.groupingBy(Request::getIp));

		List<Session> sessionList = sessions.entrySet().stream()
				.map(entry -> new WebsiteSession(entry.getKey(), entry.getValue())).filter(WebsiteSession::isValid)
	...
	private static List<Request> getRequests(String fileName) throws IOException {
		List<Request> requests = Files.lines(Paths.get(fileName))
				.filter(line -> line.indexOf("logfile turned over") == -1).map(Request::new)
				.collect(Collectors.toList());
	...
	private static boolean afterDate(String filename, String date) {
		...
		return parsed.isAfter(threshold) || parsed.isEqual(threshold);
	}

	private static void findLogFilesIncluding(String directory, String date) throws IOException {

		Files.list(Paths.get(directory)).map(path -> path.toAbsolutePath().toString())
				.filter(filename -> filename.indexOf("access.log") != -1).filter(filename -> afterDate(filename, date))
